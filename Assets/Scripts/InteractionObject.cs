...
public class InteractionObject : MonoBehaviour
{
    ...
    private void Update()
    {
        if (GameManager.Instance.ControlStart && RoomManager.Instance.PopupLayer == 0 && GameManager.Instance.CurPlayer.NormalState == ENormalState.Idle && !isFading && isExpansion 
            && Input.GetKeyDown(GameManager.Instance.upKey))
        {
    ...
    public async void FadeOut()
    {
        ...
        if (await GameManager.Instance.Fading(1, 0, 0.25f, true, ConstValues.WhiteColor).SuppressCancellationThrow())
            return;
