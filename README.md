...
```mermaid
flowchart TD
    GM[GameManager<br/>플레이어 상태·스탯·재화·세이브]
    RM[RoomManager<br/>룸 이동·카메라·미니맵]
    SM[StageManager<br/>에피소드 진행]
    CT[Controller<br/>입력]
    UM[UIManager<br/>패널 라이프사이클]
...
- `RoomManager` — 룸 간 비동기 페이드 전환
- `Stage` / `Stage1` / `Stage2` — 인트로 연출, 보스 스폰, 카메라 흔들림, 클리어 조건
- `TotalRoom` — 전체 룸 컨테이너 및 미니맵 상태 보관
...
| # | 현재 상태 | 문제 | 개선 방향 |
|---|---|---|---|
| 1 | `GameManager.cs` 3,893줄 / public 메서드 199개 | 책임이 과도하게 집중된 **God Object**. 협업 시 충돌 지점이 됨 | 세이브 / 재화 / 스탯 / 스폰 / 입력키 도메인별 분리 |
| 2 | `Room.cs` 4,469줄 | 스폰·미니맵·카메라·상호작용이 한 클래스에 혼재 | 책임별 컴포넌트로 분해 |
| 3 | `UIManager.cs` 전체 주석 처리 | MVP 인프라(인터페이스 33개)를 깔아두고 관리자를 비활성화한 상태. 팝업 생성 경로가 이원화됨 | UIManager 복구 후 팝업 생성 경로 일원화 |
| 4 | 주석 처리된 코드 약 5,200줄 | 가독성 저해, 리뷰 비용 증가 | 이력은 Git이 보관하므로 일괄 제거 |
| 5 | 스크립트 200개가 단일 폴더에 평면 배치 | 탐색 비용 | `Core / Character / Combat / UI / Room / Data / Util` 로 재구성 |
| 6 | 오브젝트 풀이 전체 리스트 선형 탐색(`FindAll`) | 풀 규모가 커질수록 스폰 비용 증가 | 프리팹 ID를 키로 하는 `Dictionary<string, Queue<GameObject>>` 로 전환 |
| 7 | 자동화 테스트 없음 | 리팩터링 시 회귀 검증 불가 | 데미지 계산·버프 만료·실드 소비 등 순수 로직부터 EditMode 테스트 도입 |

> 이 목록 자체가 다음 작업 계획입니다.
