...
| **코드 규모** | C# 스크립트 227개 / 약 41,400줄 (에디터 툴 6개 · EditMode 테스트 50개 포함) |
| **컨텐츠 규모** | 룸 51개 · 몬스터 21종 · 플레이어블 3종 |
...
**`GameObject.Find` / `FindObjectOfType` 는 전체 코드베이스에서 1회**만 사용합니다.
그 한 번도 `Singleton<T>` 베이스 클래스가 씬에 배치된 매니저를 찾는 곳이고,
나머지 참조는 전부 인스펙터 주입 또는 매니저 경유입니다.
문자열·타입 탐색은 비용도 크지만, **이름을 바꾸면 컴파일 에러 없이 조용히 깨지는** 쪽이 더 문제입니다.
...
`GetTalk` 은 코드베이스 328곳에서 호출됩니다. 이걸 전부 고치는 대신
...
Assets/Tests/       EditMode 테스트 50개 (전부)
```
